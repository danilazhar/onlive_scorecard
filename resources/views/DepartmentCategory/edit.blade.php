...
@section('title')
Edit Department Evaluation Category
@endsection
...
@section('content')
    ...
    @endif
    <form action="{{ route('department_category.postUpdate', ['id' => $category->id]) }}" method="POST">
        <div class="row">
            <div class="col-md-9">
                <div class="card">
                    <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control {{ $errors->has('department') ? 'is-invalid' : null }}" id="department" name="department">
                                        ...
                                        @foreach ($departments as $department)
                                            <option value="{{ $department->id }}" {{ $category->department_id == $department->id ? 'selected' : false }}>{{ $department->name }}</option>
                                        @endforeach                                    
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control {{ $errors->has('category') ? 'is-invalid' : null }}" id="category" name="category">
                                        ...
                                        @foreach ($categories as $categorylist)
                                            <option value="{{ $categorylist->id }}" {{ $category->category_id == $categorylist->id ? 'selected' : false }}>{{ $categorylist->name }}</option>
                                        @endforeach                                    
                                    </select>
